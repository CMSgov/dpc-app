...
start-load-tests: secure-envs
	...
	docker run --net=host --rm -v $(shell pwd)/dpc-load-testing:/src --env-file $(shell pwd)/ops/config/decrypted/local.env -e ENVIRONMENT=local -e TEST_TYPE=$$TEST_TYPE -i grafana/k6 run /src/$$TEST_TYPE.js
...
start-macaroon-tests:
	@docker run --net=host --rm -v ./dpc-load-testing:/src -e ENVIRONMENT=local -i grafana/k6 run /src/macaroonTests.js
