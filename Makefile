...
start-api-load-tests: secure-envs
	USE_BFD_MOCK=true AUTH_DISABLED=false docker compose -p dpc-load-tests up api --wait
...
start-load-tests: secure-envs
	@docker run --rm -v $(shell pwd)/dpc-load-testing:/src --env-file $(shell pwd)/ops/config/decrypted/local.env -e ENVIRONMENT=local -i grafana/k6 run /src/test.js
