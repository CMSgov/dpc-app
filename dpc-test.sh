...
docker run --rm -v $(pwd)/dpc-load-testing:/src --env-file $(pwd)/ops/config/decrypted/local.env --add-host host.docker.internal=host-gateway -e ENVIRONMENT=local -e TEST_TYPE=ci-app -i grafana/k6 run /src/ci-app.js
