...
public class JwtKeyResolver extends LocatorAdapter<Key> {
    ...
    public Key locate(JwsHeader header) {
        ...
        final Object issuer = header.get("iss");
        if (issuer == null) {
            logger.error("JWT ISS field is missing");
            throw new WebApplicationException("JWT must have ISS field", Response.Status.UNAUTHORIZED);
        }
        final UUID organizationID = getOrganizationID(issuer.toString());
