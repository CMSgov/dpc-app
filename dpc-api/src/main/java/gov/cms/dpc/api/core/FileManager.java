...
public class FileManager {
    ...
    public FilePointer getCompressedFile(UUID organizationID, String fileID) {
        ...
        final java.nio.file.Path path = Paths.get(String.format("%s/%s.ndjson.gz", fileLocation, batchFile.getFileName()));
        logger.debug("Streaming compressed file {}", path);
        return new FilePointer(Hex.toHexString(batchFile.getChecksum()),
                batchFile.getFileLength(),
                batchFile.getJobID(),
                jobQueueBatch.getStartTime().orElseThrow(() -> new IllegalStateException("Cannot find start time of completed job")),
