...
class APIResourceAnnotationTest {
    ...
    private static void assertMethodHasValidAuthAnnotations(Method method) {
        assertTrue((method.isAnnotationPresent(Authorized.class) || method.isAnnotationPresent(PathAuthorized.class) || method.isAnnotationPresent(Public.class) || method.isAnnotationPresent(AdminOperation.class)),
                String.format("Method: %s in Class: %s must either have a parameter with an Authorized, PathAuthorized, or Public annotation on the method.", method.getName(), method.getDeclaringClass())
        );
    }
}
