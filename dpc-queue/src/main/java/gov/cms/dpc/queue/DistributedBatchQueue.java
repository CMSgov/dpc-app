...
public class DistributedBatchQueue extends JobQueueCommon {
    ...
    private void restartStuckBatches(Session session) {
        // Find stuck batches
        List<String> stuckBatchIDs = session.createNativeQuery("SELECT Cast(batch_id as varchar) batch_id FROM job_queue_batch WHERE status = 1 AND update_time < current_timestamp - interval '5 minutes' FOR UPDATE SKIP LOCKED", String.class)
                .getResultList();
    ...
    private Optional<JobQueueBatch> claimBatchFromDatabase(Session session, UUID aggregatorID) {
        // Claim a new batch
        Optional<String> batchID = session.createNativeQuery("SELECT Cast(batch_id as varchar) batch_id FROM job_queue_batch WHERE status = 0 ORDER BY priority ASC, submit_time ASC LIMIT 1 FOR UPDATE SKIP LOCKED", String.class)
        ...
        try (final Session session = this.factory.openSession()) {
            try {
                Optional<Timestamp> submitTime = 
                        session.createNativeQuery("SELECT MIN( submit_time ) FROM job_queue_batch WHERE status = " + JobStatus.QUEUED.ordinal(), Timestamp.class)
                        .uniqueResultOptional();
