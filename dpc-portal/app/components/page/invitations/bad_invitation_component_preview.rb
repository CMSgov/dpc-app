  ...
  module Invitations
    ...
    class BadInvitationComponentPreview < ViewComponent::Preview
      def invalid_invitation
        ...
        reason = 'invalid'
        render(Page::Invitations::BadInvitationComponent.new(invitation, reason, 'warning'))
      end

      def pii_mismatch
        invitation = Invitation.new(provider_organization: ProviderOrganization.new(name: 'Health Hut'))
        reason = 'pii_mismatch'
        render(Page::Invitations::BadInvitationComponent.new(invitation, reason, 'error'))
      end
      ...
      def verification_failure(error_code: :user_not_authorized_official)
        ...
        render(Page::Invitations::BadInvitationComponent.new(invitation, error_code, 'error'))
      end
    end
  end
end
