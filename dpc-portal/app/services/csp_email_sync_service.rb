...
# app/services/csp_email_sync_service.rb
class CspEmailSyncService
  def initialize(csp_user)
    @csp_user = csp_user
  end

  def sync(new_emails, primary_email)
    ...
    ActiveRecord::Base.transaction do
      add_or_activate_new_email(new_emails, existing_emails)
  ...
  def deactivate_old_email(new_emails, existing_emails)
    ...
    existing_emails&.each do |existing_email|
      ...
      existing_email.update!(active: false,
