    ...
    LoginSupport::CSP_MAP.each do |csp, display_name|
      context "using #{display_name}" do
        context 'ao invalid invitation' do
          let(:component) { described_class.new(invitation, 'ao_invalid', csp) }
        ...
        context 'cd invalid invitation' do
          ...
          let(:component) { described_class.new(invitation, 'cd_invalid', csp) }
          it 'should match header' do
            header = <<~HTML
        ...
        context 'CD already accepted' do
          let(:component) { described_class.new(invitation, 'cd_accepted', csp) }
          it 'should match header' do
            header = <<~HTML
        ...
        context 'AO expired' do
          ...
          let(:component) { described_class.new(invitation, 'ao_expired', csp) }
          it 'should match header' do
            header = <<~HTML
          ...
          context 'already renewed' do
            let(:status) { :renewed }
            let(:component) { described_class.new(invitation, 'ao_renewed', csp) }
            it 'should match header' do
              header = <<~HTML
        ...
        context 'CD expired' do
          ...
          let(:component) { described_class.new(invitation, 'cd_expired', csp) }
          it 'should match header' do
            header = <<~HTML
        ...
        context 'Server error' do
          let(:component) { described_class.new(invitation, 'server_error', csp) }
          it 'should match header' do
            header = <<~HTML
