...
RSpec.describe Page::ClientToken::NewTokenComponent, type: :component do
  ...
  describe 'html' do
    ...
    let(:expected_html) do
      <<~HTML
        <div>
          <div class="margin-bottom-5">← <a href="/portal/organizations/#{org.api_id}">#{org.name}</a></div>
          <h1>Create a new client token</h1>
          <div>
            <h2>New token for #{org.name}</h2>
            <hr/>
            <form action="/portal/organizations/#{org.api_id}/client_tokens" accept-charset="UTF-8" method="post">
