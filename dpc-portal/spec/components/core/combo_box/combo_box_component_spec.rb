...
RSpec.describe Core::ComboBox::ComboBoxComponent, type: :component do
  describe 'html' do
    ...
    context 'when has one option' do
      let(:component) do
        described_class.new(label: 'label', id: 'id', options: ['option1'], on_change: 'on_change')
      end
      let(:expected_html) do
        <<~HTML
          <label class="usa-label" for="id">label</label>
    ...
    context 'when has multiple options' do
      let(:component) do
        described_class.new(label: 'label', id: 'id', options: %w[option1 option2], on_change: 'on_change')
      end
      let(:expected_html) do
        <<~HTML
          <label class="usa-label" for="id">label</label>
