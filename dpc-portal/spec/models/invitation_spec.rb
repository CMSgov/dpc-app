    ...
    describe :create do
      ...
      context 'duplicate information' do
        ...
        it 'fails on existing invitation with same email and full name' do
          ...
          expect(new_cd_invite.errors[:base].first[:status]).to eq I18n.t('errors.duplicate_cd.status')
          expect(new_cd_invite.errors[:base].first[:text]).to eq I18n.t('errors.duplicate_cd.text')
        end
