...
public class MockBlueButtonClientV2 implements BlueButtonClientV2 {
    ...
    public MockBlueButtonClientV2(@Named("fhirContextR4") FhirContext fhirContextR4) {
        fhirContextR4.setPerformanceOptions(PerformanceOptionsEnum.DEFERRED_MODEL_SCANNING);
        parser = fhirContextR4.newXmlParser();
    }
    ...
    private InputStream loadResource(String pathPrefix, String beneId) throws ResourceNotFoundException {
        ...
        return MockBlueButtonClientV2.class.getClassLoader().getResourceAsStream(path);
    }
    ...
    private Bundle loadEmptyBundle() {
        try(InputStream sampleData = MockBlueButtonClientV2.class.getClassLoader().getResourceAsStream(SAMPLE_EMPTY_BUNDLE)) {
            final var bundle = parser.parseResource(Bundle.class, sampleData);
            bundle.getMeta().setLastUpdated(Date.from(BFD_TRANSACTION_TIME.toInstant()));
            return bundle;
        } catch(IOException ex) {
