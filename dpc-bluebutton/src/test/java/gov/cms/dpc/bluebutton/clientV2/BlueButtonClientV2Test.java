...
class BlueButtonClientV2Test {
    ...
    private static final String METADATA_PATH = "bb-test-data-v2/meta.xml";
    ...
    static void setupBlueButtonClient() throws IOException {
        ...
        createMockServerExpectation("/v2/fhir/metadata", HttpStatus.OK_200, getRawXML(METADATA_PATH), List.of());
    ...
    void shouldGetEOBFromPatientID() {
        ...
        assertEquals(38, response.getTotal(), "The demo patient should have exactly 38 EOBs");
    }
    ...
    private static BBClientConfiguration getClientConfig() {
        ...
        return yaml.load(BlueButtonClientV2Test.class.getClassLoader().getResourceAsStream("testv2.application.yml"));
    }

    private static String getRawXML(String path) throws IOException {
        InputStream sampleData = BlueButtonClientV2Test.class.getClassLoader().getResourceAsStream(path);

        if (sampleData == null) {
            throw new MissingResourceException("Cannot find sample requests", BlueButtonClientV2Test.class.getName(), path);
        }
