...
public class LookBackServiceTest {
    ...
    public void beforeEach() {
        ...
        lookBackService = new LookBackService(rosterDAO, operationsConfig);
    ...
    public void testClaimWithinTimeFrame() {
        ...
        boolean result = lookBackService.hasClaimWithin(eob, orgID, providerID.toString(), 1);
        Assertions.assertTrue(result);

        dateTime = OffsetDateTime.now(ZoneOffset.UTC).minusMonths(1);
        eob.getBillablePeriod().setEnd(Date.from(dateTime.toInstant()));

        result = lookBackService.hasClaimWithin(eob, orgID, providerID.toString(), 1);
    ...
    public void testNonMatchingOrgID() {
        ...
        boolean result = lookBackService.hasClaimWithin(eob, UUID.randomUUID(), providerID.toString(), 1);
        Assertions.assertFalse(result);
    }
