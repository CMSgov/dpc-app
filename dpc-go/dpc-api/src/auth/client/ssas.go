...
import (
	...
	"github.com/CMSgov/dpc/api/config"
...
func NewSSASClient() (*SSASClient, error) {
	...
	if config.GetEnv("SSAS_USE_TLS") == "true" {
	...
	var timeout int
	if timeout, err = strconv.Atoi(config.GetEnv("SSAS_TIMEOUT_MS")); err != nil {
...
func tlsTransport() (*http.Transport, error) {
	caFile := config.GetEnv("BCDA_CA_FILE")
...
func (c *SSASClient) VerifyPublicToken(tokenString string) ([]byte, error) {
	...
	clientID := config.GetEnv("BCDA_SSAS_CLIENT_ID")
	secret := config.GetEnv("BCDA_SSAS_SECRET")
	if clientID == "" || secret == "" {
