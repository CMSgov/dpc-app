...
class ProvenanceResourceFactoryTest {

    private static final FhirContext ctx = FhirContext.forDstu3();

    private ProvenanceResourceFactoryTest() {
        // Not used
    }
    ...
    void testValidProvenance() {
        ...
        Mockito.when(mock.getHeader(ProvenanceResourceFactory.PROVENANCE_HEADER)).thenReturn(provString);
        ...
        final ProvenanceResourceFactory factory = new ProvenanceResourceFactory(mockInjector, ctx);
    ...
    void testMissingHeader() {
        final HttpServletRequest mock = Mockito.mock(HttpServletRequest.class);
        Mockito.when(mock.getHeader(ProvenanceResourceFactory.PROVENANCE_HEADER)).thenReturn(null);
        ...
        assertAll(() -> assertEquals(HttpStatus.BAD_REQUEST_400, exception.getResponse().getStatus(), "Should be a bad request"),
                () -> assertEquals(String.format("Must have %s header", ProvenanceResourceFactory.PROVENANCE_HEADER), exception.getMessage(), "Should show missing header"));
    }
