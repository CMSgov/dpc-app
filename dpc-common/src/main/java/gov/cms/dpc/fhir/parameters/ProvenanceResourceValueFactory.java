...
/**
 * {@link Factory} for extracting a {@link org.hl7.fhir.dstu3.model.Provenance}
 * resource from a request header.
 */
public class ProvenanceResourceValueFactory implements Factory<Provenance> {
    ...
    ProvenanceResourceValueFactory(ContainerRequest request, FhirContext ctx) {
    ...
    public Provenance provide() {
        final String headerValue = request.getHeaderString(PROVENANCE_HEADER);
        if (headerValue == null) {
            throw new WebApplicationException(String.format("Must have %s header", PROVENANCE_HEADER),
