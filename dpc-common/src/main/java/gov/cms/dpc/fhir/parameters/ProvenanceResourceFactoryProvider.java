...
/**
 * Custom {@link ValueParamProvider} that lets us cleanly extract
 * {@link org.hl7.fhir.dstu3.model.Provenance} resources from the
 * {@link ProvenanceResourceValueFactory#PROVENANCE_HEADER}.
 */
@Provider
public class ProvenanceResourceFactoryProvider implements ValueParamProvider {
    ...
    @Inject
    public ProvenanceResourceFactoryProvider(FhirContext ctx) {
        this.ctx = ctx;
    }
    ...
    public Function<ContainerRequest, Provenance> getValueProvider(Parameter parameter) {
        if (parameter.getDeclaredAnnotation(ProvenanceHeader.class) != null) {
            ...
            if (IBaseResource.class.isAssignableFrom(parameter.getRawType())) {
                return x -> new ProvenanceResourceValueFactory(x, ctx).provide();
            }
        }
